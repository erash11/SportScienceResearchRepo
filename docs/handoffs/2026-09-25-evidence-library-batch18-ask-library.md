...
- A separate SCN sprint-course Tier A note at `C:\Users\eric_rash\.claude\projects\C--Users-eric-rash-Desktop-DEV\memory\project_scn-sprint-course.md` reported SecondBrain vault commit `23d0ad0` unpushed. The initial search checked user-home and Drive paths but missed `C:\Users\eric_rash\Desktop\SecondBrain`.

## Vault push resolution — 2026-09-26

- The Desktop SecondBrain checkout contained `23d0ad0` plus documentation commit `41a7f8d`, with a clean working tree and two commits ahead of `origin/main`. `git pull --no-rebase` incorporated remote weekly-slate commit `1996afd`, producing merge commit `1874eb0`; `git push` succeeded. `git merge-base --is-ancestor 23d0ad0 origin/main` passed, GitHub resolves the full commit `23d0ad0157b503060bbd09f2ed79046d4f2c48ca`, and the checkout is clean against origin. The vault library entries are now available through GitHub.
